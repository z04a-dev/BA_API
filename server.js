...
function getOccurence(info, callback) {
	pool.getConnection(function(err,con) {
		if(err) throw err;
		con.query("SELECT * FROM info WHERE name='" + info + "'", function (err, result) {
			if (err) throw err;
			if (result.length == 0 && !blockedarr.includes(info) ) {
				callback(err, "ok")
			} else {
				callback(err, 'fail')
			}
...
function newInfo(info, callback) {
	pool.getConnection(function(err,con) {
		if(err) throw err;
		con.query("INSERT INTO info VALUES ('" + MD5(info) + "', '" + info + "');", function (err, result) {
			if (err) throw err;
			console.log("INSERTED NEW DATA: " + info)
			callback(err, "ok")
		})
		con.release()
	})
...
app.post('/generator', (req, res) => {
	...
	if(req.headers['info'] != undefined) {
		...
		var fs = require('fs');
		var stream = fs.createWriteStream("tmp/" + libraryName + ".txt");
		stream.once('open', function(fd) {
			resultArray.forEach(element => {
				if(resultArray.indexOf(element) != resultArray.length-1) {
					stream.write(transliterate(element).replace(/\s+/g, '') + " " + element + "\n");
				} else {
					stream.write(transliterate(element).replace(/\s+/g, '') + " " + element);
				}
			});
